...
import { Calendar, Home, Info, Menu, MessageSquare, Tag, Zap } from "lucide-react";
            ...
            {links.map(([v,l,Icon]) => (
              <button key={v} onClick={() => onNav(v)}
                style={{ display:"flex",alignItems:"center",gap:6,padding:"7px 14px",borderRadius:10,
                  background:view===v?"var(--ev-accent)20":"none",border:"none",
                  color:view===v?"var(--ev-accentL)":"var(--ev-muted)",fontSize:14,fontWeight:600,cursor:"pointer",transition:"all .15s" }}
                onMouseEnter={e=>{if(view!==v){e.currentTarget.style.color=T.text;e.currentTarget.style.background="var(--ev-glass)"}}}
