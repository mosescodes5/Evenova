...
import {
  Activity, BarChart3, Briefcase, Building, Calendar, CreditCard,
  DollarSign, LayoutDashboard, LogOut, Menu, MessageCircle, Scan,
  Sparkles, List, Users, Zap,
} from "lucide-react";
...
export default function AppNav({ user, onNav, onLogout }) {
  ...
  const orgLinks = [
    ...
    ["payment-settings", "Payments",   CreditCard],
    ["team",             "Team",       Users],
    ["scanner",          "Scanner",    Scan],
    ["live",             "Live",       Activity],
    ["scan-log",         "Scan Log",   List],
    ["email-blast",      "Email",      Zap],
  ...
  const adminLinks = [
    ...
    ["admin-orgs",     "Organizers", Building],
    ["admin-events",   "Events",     Calendar],
    ["admin-revenue",  "Revenue",    DollarSign],
    ["admin-payouts",  "Payouts",    CreditCard],
    ["admin-scan-log", "Scan Log",   List],
    ["email-blast",    "Email",      Zap],
    ["whatsapp-blast", "WhatsApp",   MessageCircle],
            ...
            {links.map(([v, l, Icon]) => (
              <button key={v} onClick={() => onNav(v)}
                style={{
                  ...
                  color: "var(--ev-muted)", fontSize: 13, fontWeight: 600,
                  cursor: "pointer", whiteSpace: "nowrap",
                }}
                onMouseEnter={e => { e.currentTarget.style.color = "var(--ev-text)"; e.currentTarget.style.background = "var(--ev-glass)"; }}
