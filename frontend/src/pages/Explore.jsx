...
export default function Explore({ events, onEventPage }) {
  ...
  const [search, setSearch] = useState(""), [cat, setCat] = useState("All"), [sort, setSort] = useState("date");
  ...
  const filtered = useMemo(()=>{
    ...
    if (sort==="price") list=[...list].sort((a,b)=>Math.min(...Object.values(a.ticketTypes).map(t=>t.price))-Math.min(...Object.values(b.ticketTypes).map(t=>t.price)));
    return list;
  },[events,search,cat,sort]);

  return (
    <div style={{background:T.bg,paddingTop:64,minHeight:"100vh"}}>
      <div style={{background:T.surface,borderBottom:`1px solid ${T.border}`,padding:mobile?"24px 16px":"32px 24px"}}>
        <div style={{maxWidth:1200,margin:"0 auto"}}>
          <h1 className="outfit" style={{fontSize:mobile?26:34,fontWeight:800,color:T.text,marginBottom:20}}>All Events</h1>
          <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
            <div style={{display:"flex",alignItems:"center",gap:8,flex:1,minWidth:200,padding:"10px 14px",
      ...
      <div style={{maxWidth:1200,margin:"0 auto",padding:mobile?"24px 16px":"40px 24px"}}>
        <p style={{fontSize:13,color:T.muted,marginBottom:20}}>{filtered.length} event{filtered.length!==1?"s":""} found</p>
        <div className="ga">
          {filtered.map(ev=>{
...
/* ─────────────────────────────────────────────────────────────
   16. PUBLIC EVENT PAGE  (Registration)
───────────────────────────────────────────────────────────── */
