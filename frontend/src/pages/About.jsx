...
import { Reveal, StaggerReveal, StaggerItem, PageMotion, HoverCard, fadeUp, scaleIn } from "../components/Motion.jsx";
...
export default function About({ onNav }) {
  ...
  return (
    <PageMotion><div style={{background:"#08080f",paddingTop:64}}>
      <div style={{background:`linear-gradient(135deg,${T.accent}20,transparent)`,borderBottom:`1px solid ${T.border}`,padding:mobile?"60px 20px":"80px 24px",textAlign:"center"}}>
        <motion.h1 initial={{opacity:0,y:24}} animate={{opacity:1,y:0}} transition={{duration:.7}} className="outfit" style={{fontSize:mobile?36:52,fontWeight:900,color:T.text,marginBottom:16}}>Built for African Events</motion.h1>
      ...
      <div style={{maxWidth:1100,margin:"0 auto",padding:mobile?"60px 16px":"80px 24px"}}>
        <StaggerReveal stagger={0.1} className="g2">
          {vals.map(([Icon,t,d],i)=>(
            <StaggerItem key={i}>
            <HoverCard style={{padding:28,borderRadius:16,background:"rgba(255,255,255,0.03)",backdropFilter:"blur(20px)",border:"1px solid rgba(255,255,255,0.08)"}}>
